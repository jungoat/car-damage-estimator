...
class COCOFormatter:
    def __init__(self, task, csv_path, img_dir, ann_dir, output_dir, labeling_scheme, file_column):
        # 데이터셋 기본 설정값 저장
        self.task = task              # 'damage' 또는 'part'
        self.csv_path = csv_path      # CSV 파일 경로
        self.img_dir = img_dir        # 이미지 폴더
        self.ann_dir = ann_dir        # 어노테이션 폴더
        self.output_dir = output_dir  # 변환 후 저장할 폴더
        self.labeling_scheme = labeling_scheme  # 라벨 이름 리스트
        self.file_column = file_column          # 파일명 컬럼명
    ...
    def process_subset(self, subset, alias):
        # subset: train/val/test, alias: 저장 시 사용할 이름
        df = pd.read_csv(self.csv_path)

        # damage는 어노테이션 없는 경우 제외
        if self.task == "damage":
            df = df[df.total_anns > 0]

        # subset에 해당하는 파일 목록 가져오기
        ...
        if not annotations:
            print(f"'{self.ann_dir}' json 없음")
            return

        # 첫 번째 어노테이션 파일을 베이스로 사용
        ...
        print(f"{alias}: images={len(base_json['images'])}, annotations={len(base_json['annotations'])}")
...
def label_accuracy_score(hist):
    acc = np.diag(hist).sum() / hist.sum()  # 전체 픽셀 정확도
    acc_cls = np.nanmean(np.diag(hist) / hist.sum(axis=1))  # 클래스별 평균 정확도
    iu = np.diag(hist) / (hist.sum(axis=1) + hist.sum(axis=0) - np.diag(hist))  # IoU
    mean_iu = 0.0 if np.isnan(iu).all() else np.nanmean(iu)  # 평균 IoU
    cls_iu = np.where(np.isnan(iu), -1, iu)  # NaN은 -1 처리
    freq = hist.sum(axis=1) / hist.sum()  # 클래스 비율
    fwavacc = (freq[freq > 0] * iu[freq > 0]).sum()  # 가중 평균 정확도
    return acc, acc_cls, mean_iu, fwavacc, cls_iu

def _fast_hist(true, pred, n_class):
    # 빠른 confusion matrix 계산
...
if __name__ == "__main__":
    ...
    if args.task == 'damage':
        ...
        labeling_scheme = [
            ...
            "Rear fender(R)", "Trunk lid", "Bonnet", "Rear fender(L)",
            "Rear door(R)", "Head lights(R)", "Head lights(L)", "Front Wheel(R)",
            "Front door(R)", "Side mirror(R)"
        ]
        file_column = "img_id"

    # train/val/test 각각 COCO 포맷 변환
